...
import { isNumExp, isBoolExp, isVarRef, isPrimOp, isProgram, isDefineExp, isVarDecl,
         ...
         isClassExp, parseL5Exp, unparse, Exp, parseL5, Program, parsedToClassExps } from "../src/L51-ast";
...
describe('L5 Parser', () => {
    ...
    it ('parses class expressions with more than one fields (added)', () => {
        //console.log("%j", p("(class ((a : number) (b : nummber)) ((first (lambda () : number a))))"))
        expect(p("(class ((a : number) (b : nummber)) ((first (lambda () : number a))))")).to.satisfy(isOkT(isClassExp));
    })

    it ('parses class expressions with more then one field and more than one method (added)', () => {
        //console.log("%j", p("(class ((a : number) (b : nummber)) ((first (lambda () : number a)) (second (lambda () : number a))))"))
        expect(p("(class ((a : number) (b : nummber)) ((first (lambda () : number a)) (second (lambda () : number a))))")).to.satisfy(isOkT(isClassExp));
    })
    ...
    it('performs parsedtoclassExps on a program (added)', () => {
        bind(parseL5("(L5 (class ((a : number)) ((first (lambda () : number a)))))"), 
                        (parsed: Program) => {
                            //console.log("%j", parsedToClassExps(parsed))
                            return makeOk(5);       // I just want to see what is printed on the console.
                        });
